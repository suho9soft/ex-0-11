...
def toggle_led(index):
    ...
    topic = f"arduino/led{index+1}"
    client.publish(topic, payload)

def on_connect(client, userdata, flags, rc):
    print("[MQTT] 연결됨" if rc == 0 else f"[MQTT] 연결 실패 - 코드 {rc}")
    ...
    def update_from_msg():
        ...
        for i in range(8):
            if topic == f"arduino/led{i+1}":
                ...
                leds[i].on() if led_states[i] else leds[i].off()

        update_ui()

    window.after(0, update_from_msg)
