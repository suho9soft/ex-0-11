...
# ======================== 4. UI 업데이트 ========================
def update_ui():
    temp_label.config(text=f"현재 온도: {current_values['temp']:.1f} °C")
    humi_label.config(text=f"현재 습도: {current_values['humi']:.1f} %")
    pot_label.config(text=f"가변저항 값: {current_values['pot']}")
    relay_label.config(
...
temp_label = tk.Label(sensor_frame, text="현재 온도: -- °C", font=LABEL_FONT, bg=FRAME_BG, fg=LABEL_COLOR)
humi_label = tk.Label(sensor_frame, text="현재 습도: -- %", font=LABEL_FONT, bg=FRAME_BG, fg=LABEL_COLOR)
pot_label = tk.Label(sensor_frame, text="가변저항 값: --", font=LABEL_FONT, bg=FRAME_BG, fg=LABEL_COLOR)
...
relay_label = tk.Label(window, text="릴레이 상태: OFF", font=LABEL_FONT, bg="#1E1F26", fg="#E74C3C")
