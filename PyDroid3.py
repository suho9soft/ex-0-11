...
class IoTDashboard(BoxLayout):
    def __init__(self, **kwargs):
        ...
        font = {"font_name": "NotoSans"}

        # 날짜/시간
        self.date_label = Label(font_size=sp(18), size_hint_y=None, height=dp(30), **font)
        self.time_label = Label(font_size=sp(18), size_hint_y=None, height=dp(30), **font)
        self.add_widget(self.date_label)
        self.add_widget(self.time_label)

        # 센서 데이터
        self.temp_label = Label(text="온도: -- °C", font_size=sp(16), size_hint_y=None, height=dp(30), **font)
        self.humi_label = Label(text="습도: -- %", font_size=sp(16), size_hint_y=None, height=dp(30), **font)
        self.pot_label = Label(text="가변저항: --", font_size=sp(16), size_hint_y=None, height=dp(30), **font)
    ...
    def on_message(self, client, userdata, msg):
        ...
        if topic=="arduino/input":
            ...
            self.relay = (payload.strip().upper()=="ON"); self.update_relay()
        else:
            for i in range(8):
                if topic==f"arduino/led{i+1}":
                    state=(payload=="1")
                    self.led_states[i]=state
                    self.led_buttons[i].background_color = (0.2,1,0.2,1) if state else (0.7,0.7,0.7,1)
    ...
    def make_led_callback(self, idx):
        def cb(instance):
            st = not self.led_states[idx]; self.led_states[idx]=st
            self.client.publish(f"arduino/led{idx+1}", "1" if st else "0")
            instance.opacity=0.6
            Animation(opacity=1, duration=0.1).start(instance)
        return cb
